...
const getClareCountyCourses = cache(async () => {
...
export async function generateMetadata(): Promise<Metadata> {
  const { courses } = await getClareCountyCourses();
  const courseCount = courses.length;

  const title = `${courseCount} Golf Courses in Clare | Access & Prices`;

  const description = `Explore ${courseCount} visitor-friendly golf courses in County Clare, including Lahinch and Doonbeg. Compare guest access, prices and course types.`;
  ...
  return {
    ...
    alternates: {
      canonical: "/golf-courses-clare",
    },

    openGraph: {
      title,
      description,
      url: `${siteUrl}/golf-courses-clare`,
  ...
  return (
    <>
      <h2 className="text-lg font-semibold text-slate-900">
        Continue planning your Clare golf trip
      </h2>

      <p className="mt-3 text-sm leading-6 text-slate-600">
        Use Clare as your base, explore nearby golf outside the county, or build
...
export default async function GolfCoursesClarePage() {
  const { courses, error } = await getClareCountyCourses();
        ...
        <div className="mx-auto max-w-[480px] lg:max-w-[1120px]">
          ...
          <div className="mt-6 lg:max-w-[800px]">
            ...
            <p className="mt-4 text-[15px] leading-6 text-emerald-50/95 lg:max-w-[740px] lg:text-[17px] lg:leading-7">
              ...
              County Clare, with clear visitor access, pricing and course
              information.
            </p>

            <p className="mt-3 text-[15px] leading-6 text-emerald-50/95 lg:max-w-[740px] lg:text-[17px] lg:leading-7">
              Compare courses across Clare, shortlist where you want to play and
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <span className="inline-block rounded-full bg-white/15 px-4 py-2 text-sm font-semibold text-white backdrop-blur">
                {courseCount} courses in County Clare
              </span>
        ...
        <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-stretch">
          <section className="min-w-0 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 lg:flex lg:h-full lg:flex-col lg:p-7">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-emerald-800">
              County Clare golf guide
            </p>

            <h2 className="mt-1 text-[21px] font-semibold text-slate-900 lg:text-[24px]">
              Golf in Clare — The Banner County
            </h2>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              County Clare is made for a golf trip, but the golf is only part of
              the reason to go. The Wild Atlantic Way runs along the county&apos;s
              west coast, passing the Cliffs of Moher and Lahinch — a town where
              surfboards and golf bags are equally at home.
            </p>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              Golf has long been part of the county&apos;s identity. Doonbeg hosted
              the 2026 Irish Open, while Lahinch remains one of Ireland&apos;s
              best-known links. Away from the coast, Dromoland Castle, Ennis and
              Shannon offer a very different parkland test.
            </p>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              Across the Banner County, GuestPlayGolf lists {courseCount}{" "}
              visitor-friendly golf courses, ranging from local parkland clubs
              to resort golf and famous coastal links. Compare where you can
              play, what it costs and the type of course, then build the ones you
              like into your Clare golf trip.
            </p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Every listed visitor-friendly course located in County Clare.
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  See when independent guests can play each Clare course.
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Use price bands to compare Clare courses before planning.
                </p>
              </div>
            </div>
          </section>
          ...
          <aside className="min-w-0">
            <div className="flex h-full flex-col rounded-3xl bg-emerald-50 p-5 shadow-sm ring-1 ring-emerald-100 lg:p-6">
              ...
              <h2 className="mt-4 text-xl font-bold text-slate-900">
                Build your Clare golf trip
              </h2>

              <p className="mt-3 text-sm leading-6 text-slate-700">
                Choose Clare courses, build a day-by-day itinerary and organise
              ...
              <div className="mt-4 grid gap-3">
                <div className="flex gap-3">
                  ...
                  <p className="pt-1 text-sm text-slate-700">
                    Compare the Clare courses below.
                  </p>
                </div>
              ...
              <div className="mt-auto pt-5">
                ...
                <p className="mt-3 text-center text-xs leading-5 text-slate-600">
                  Browse Clare courses below and add your preferred options as
                  you go.
                </p>
              </div>
            </div>
          ...
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold text-slate-900 lg:text-2xl">
                {courseCount} golf courses in Clare
              </h2>
            </div>
            ...
            >
              Looking beyond the county? See Golf Near Adare Manor →
            </Link>
          </div>

          {error && (
            <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              Error loading golf courses in Clare.
            </div>
          )}

          {courses.length === 0 ? (
            <div className="rounded-2xl bg-white p-5 text-sm text-slate-600 shadow-sm ring-1 ring-slate-200/70">
              No golf courses are currently listed in County Clare.
            </div>
          ) : (

...
const getKerryCountyCourses = cache(async () => {
...
export async function generateMetadata(): Promise<Metadata> {
  const { courses } = await getKerryCountyCourses();
  const courseCount = courses.length;

  const title = `${courseCount} Golf Courses in Kerry | Access & Prices`;

  const description = `Explore ${courseCount} visitor-friendly golf courses in County Kerry, Ireland. Compare visitor access, prices and course types, then build and share a free Kerry golf trip itinerary.`;
  ...
  return {
    ...
    alternates: {
      canonical: "/golf-courses-kerry",
    },

    openGraph: {
      title,
      description,
      url: `${siteUrl}/golf-courses-kerry`,
  ...
  return (
    <>
      <h2 className="text-lg font-semibold text-slate-900">
        Continue planning your Kerry golf trip
      </h2>

      <p className="mt-3 text-sm leading-6 text-slate-600">
        Explore more golf around Ireland or build a full Kerry golf itinerary
        with GuestPlayGolf.
      </p>
...
export default async function GolfCoursesKerryPage() {
  const { courses, error } = await getKerryCountyCourses();
        ...
        <div className="mx-auto max-w-[480px] lg:max-w-[1120px]">
          ...
          <div className="mt-6 lg:max-w-[800px]">
            ...
            <p className="mt-4 text-[15px] leading-6 text-emerald-50/95 lg:max-w-[740px] lg:text-[17px] lg:leading-7">
              ...
              County Kerry, with clear visitor access, pricing and course
              information.
            </p>

            <p className="mt-3 text-[15px] leading-6 text-emerald-50/95 lg:max-w-[740px] lg:text-[17px] lg:leading-7">
              Compare courses across Kerry, shortlist where you want to play
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <span className="inline-block rounded-full bg-white/15 px-4 py-2 text-sm font-semibold text-white backdrop-blur">
                {courseCount} courses in County Kerry
              </span>
        ...
        <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-stretch">
          <section className="min-w-0 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 lg:flex lg:h-full lg:flex-col lg:p-7">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-emerald-800">
              County Kerry golf guide
            </p>

            <h2 className="mt-1 text-[21px] font-semibold text-slate-900 lg:text-[24px]">
              Golf in Kerry — The Kingdom
            </h2>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              Known as &quot;The Kingdom&quot;, County Kerry is one of
              Ireland&apos;s most famous destinations and one of its great golf
              counties. The landscape is dramatic even by Irish standards,
              with mountains, Atlantic coastline, beaches and small towns
              spread across the southwest of the country.
            </p>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              Killarney is the natural starting point for many visitors,
              combining golf with Killarney National Park and easy access to
              the Ring of Kerry. Further west, the Dingle Peninsula offers
              another spectacular stretch of coastline, with Dingle town
              making a great stop for food, pubs and a night away from the
              course.
            </p>

            <p className="mt-4 text-sm leading-7 text-slate-600">
              Kerry boasts {courseCount} visitor-friendly golf courses on
              GuestPlayGolf, including some of Ireland&apos;s best-known links
              alongside parkland and resort golf. A Kerry golf trip can easily
              combine Killarney, the Ring of Kerry, Dingle and the Atlantic
              coast, with the drives between courses often becoming as
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Every listed visitor-friendly course located in County Kerry.
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  See when independent guests can play each Kerry course.
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Use price bands to compare Kerry courses before planning.
                </p>
              </div>
            </div>
          </section>
          ...
          <aside className="min-w-0">
            <div className="flex h-full flex-col rounded-3xl bg-emerald-50 p-5 shadow-sm ring-1 ring-emerald-100 lg:p-6">
              ...
              <h2 className="mt-4 text-xl font-bold text-slate-900">
                Build your Kerry golf trip
              </h2>

              <p className="mt-3 text-sm leading-6 text-slate-700">
                Choose Kerry courses, build a day-by-day itinerary and organise
              ...
              <div className="mt-4 grid gap-3">
                <div className="flex gap-3">
                  ...
                  <p className="pt-1 text-sm text-slate-700">
                    Compare the Kerry courses below.
                  </p>
                </div>
              ...
              <div className="mt-auto pt-5">
                ...
                <p className="mt-3 text-center text-xs leading-5 text-slate-600">
                  Browse Kerry courses below and add your preferred options as
                  you go.
                </p>
              </div>
            </div>
          ...
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold text-slate-900 lg:text-2xl">
                {courseCount} golf courses in Kerry
              </h2>
            </div>
            ...
            >
              Looking for more links golf? See Links Golf in Ireland →
            </Link>
          </div>

          {error && (
            <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              Error loading golf courses in Kerry.
            </div>
          )}

          {courses.length === 0 ? (
            <div className="rounded-2xl bg-white p-5 text-sm text-slate-600 shadow-sm ring-1 ring-slate-200/70">
              No golf courses are currently listed in County Kerry.
            </div>
          ) : (
